-- Trip Photos Database Setup - CLEAN INSTALL
-- This script safely drops and recreates everything

-- Drop existing policies first (if they exist)
DROP POLICY IF EXISTS "Users can view own photos" ON trip_photos;
DROP POLICY IF EXISTS "Users can upload photos to accessible trips" ON trip_photos;
DROP POLICY IF EXISTS "Users can update own photos" ON trip_photos;
DROP POLICY IF EXISTS "Users can delete own photos" ON trip_photos;

-- Drop storage policies (if they exist)
DROP POLICY IF EXISTS "Authenticated users can upload photos" ON storage.objects;
DROP POLICY IF EXISTS "Anyone can view photos" ON storage.objects;
DROP POLICY IF EXISTS "Users can update own photos in storage" ON storage.objects;
DROP POLICY IF EXISTS "Users can delete own photos in storage" ON storage.objects;

-- Drop trigger and function (if they exist)
DROP TRIGGER IF EXISTS trip_photos_updated_at ON trip_photos;
DROP FUNCTION IF EXISTS update_trip_photos_updated_at();

-- Drop table (if exists) - WARNING: This deletes all photo records!
-- Comment out the next line if you want to keep existing data
-- DROP TABLE IF EXISTS trip_photos CASCADE;

-- Create trip_photos table (only if it doesn't exist)
...
DO $$
BEGIN
  RAISE NOTICE 'Photo map setup completed successfully!';
  RAISE NOTICE 'Table: trip_photos created';
  RAISE NOTICE 'Storage bucket: trip-photos created';
  RAISE NOTICE 'RLS policies applied';
  RAISE NOTICE 'Ready to upload photos!';
END $$;
